...
def export_clip(clip, delete_videos, low_res, download_path, frame_size, split, fps=None, use_cuda=True):
    """
    ...
    Args:
        ...
        split (str): The split the clip belongs to (train, valid, test, or challenge).
        fps (int | None): Frames per second to export. None exports all frames at native fps.
        use_cuda (bool): Whether to use CUDA hardware acceleration.
    ...
    # Build the video filter string, prepending fps filter when requested
    fps_prefix = f"fps={fps}," if fps is not None else ""
    if frame_size == "448p":
        vf = f"{fps_prefix}scale=796x448"
    else:
        vf = f"{fps_prefix}scale=398x224"

    # Change the resolution of the video if using 448p. Otherwise, use the original resolution
    if use_cuda:
        subprocess.call(["ffmpeg", "-hwaccel", "cuda", "-i", video_path, "-q:v", "1", "-vf", vf, os.path.join(video_path.parent, "frame%d.jpg")])
    else:
        subprocess.call(["ffmpeg", "-i", video_path, "-q:v", "1", "-vf", vf, os.path.join(video_path.parent, "frame%d.jpg")])
...
def export_frames(split, download_path, delete_videos=False, frame_size="448p", num_cpus=4, fps=None, use_cuda=True):
    """
    ...
    with mp.Pool(num_cpus) as p:
        p.starmap(export_clip, zip(clips, repeat(delete_videos), repeat(low_res), repeat(download_path), repeat(frame_size), repeat(split), repeat(fps), repeat(use_cuda)))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Frames per second to export. If not set, all frames are exported at the native video fps.",
    )
    parser.add_argument(
        "--cpus",
        type=int,
    ...
    for split in splits:
        ...
        export_frames(split, os.path.join(args.download_path, frame_size_path), args.delete_videos, args.frame_size, args.cpus, args.fps, args.use_cuda)
